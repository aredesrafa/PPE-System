import { c as create_ssr_component, a as compute_rest_props, v as validate_component, b as compute_slots, h as escape } from "./ssr.js";
import { f as fade } from "./index2.js";
import { F as Frame, B as Button } from "./Button.js";
import { twMerge } from "tailwind-merge";
import { C as CloseButton } from "./CloseButton.js";
import { A as ArrowRightOutline } from "./ArrowRightOutline.js";
import { E as ExclamationCircleOutline } from "./ExclamationCircleOutline.js";
...
function generateAdditionalColaboradores() {
  ...
  return nomes.map((nome, index) => {
    ...
    return {
      id,
      nome,
      cpf: `${(1e8 + index * 111111).toString().substring(0, 11)}`,
...
function generateAdditionalFichas() {
  const additionalColaboradores = generateAdditionalColaboradores();
  const statusOptions = ["ativo", "vencido", "suspenso"];
  return additionalColaboradores.filter((col) => col.temFichaAtiva).map((col, index) => {
    ...
    return {
      ...
      status: validadeDate < /* @__PURE__ */ new Date() ? "vencido" : statusOptions[Math.floor(Math.random() * statusOptions.length)],
...
function simulateNetworkDelay(min = 300, max = 1e3) {
...
export {
  ErrorDisplay as E,
  simulateNetworkError as a,
  colaboradoresMock as c,
  entregasMock as e,
  fichasEPIMock as f,
  generateId as g,
  itenEstoqueMock as i,
  simulateNetworkDelay as s,
  tiposEPIMock as t
};
