import { c as create_ssr_component, a as compute_rest_props, j as getContext, d as spread, g as add_attribute, h as escape, f as escape_attribute_value, e as escape_object, k as subscribe, v as validate_component, l as each, m as missing_component } from "../../chunks/ssr.js";
...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  const activitiesStore = writable([
    ...
    },
    {
      id: 4,
      type: "vencimento",
      description: "EPI próximo ao vencimento",
      time: "há 1 dia",
      equipment: "Máscara PFF2 - Pedro Oliveira"
    },
    {
      id: 5,
      type: "ficha",
      description: "Nova ficha EPI criada",
      time: "há 1 dia",
      equipment: "Ana Costa - Administrativa"
    }
  ]);
  ...
  return `${$$result.head += `<!-- HEAD_svelte-s5dteg_START -->${$$result.title = `<title>Dashboard - DataLife EPI</title>`, ""}<!-- HEAD_svelte-s5dteg_END -->`, ""} <div class="space-y-6"> <div class="flex items-center justify-between"><div data-svelte-h="svelte-sn35tp"><h1 class="text-2xl font-bold text-gray-900 dark:text-white">Dashboard</h1> <p class="text-sm text-gray-600 dark:text-gray-400">Visão geral do sistema de gerenciamento de EPIs</p></div> <div class="flex space-x-2">${validate_component(Button, "Button").$$render(
    $$result,
    {
      size: "sm",
      color: "primary",
  ...
  )}</div></div>  <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">${$loading ? `${each(Array(4), (_, i) => {
  ...
  })}`}</div>  <div class="grid grid-cols-1 lg:grid-cols-3 gap-6"> <div class="lg:col-span-2">${validate_component(Card, "Card").$$render($$result, { size: "sm", class: "rounded-sm" }, {}, {
    default: () => {
    ...
    default: () => {
      return `<h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4" data-svelte-h="svelte-128rda8">Ações Rápidas</h3> <div class="space-y-3">${validate_component(Button, "Button").$$render(
        $$result,
        {
          color: "primary",
