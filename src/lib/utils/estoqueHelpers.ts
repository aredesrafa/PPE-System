...
import { itenEstoqueAPI, tiposEPIAPI } from '$lib/services/api';
import { isDataVencida, isProximoVencimento as isProximoVencimentoDate } from './dateHelpers';

// Calcular status do item de estoque
export const calcularStatusEstoque = (item: ItemEstoque): 'disponivel' | 'baixo' | 'vencendo' | 'vencido' | 'esgotado' => {
...
export const registrarEventoEstoque = async (evento: Partial<EventoEstoque>) => {
  try {
...
export const verificarAlertasEstoque = async (): Promise<{
  ...
  try {
    const todosItens = await itenEstoqueAPI.getAll();
