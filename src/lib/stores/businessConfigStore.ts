...
function createBusinessConfigStore() {
  const { subscribe, set, update } = writable<BusinessConfigState>(initialState);
...
export const tiposMovimentacaoStore: Readable<BusinessConfigurationItem[]> = derived(
  businessConfigStore,
  $config => $config.data?.tiposMovimentacao || []
);

export const categoriasEPIStore: Readable<BusinessConfigurationItem[]> = derived(
  businessConfigStore,
  $config => $config.data?.categoriasEPI || []
);

export const statusEntregaStore: Readable<BusinessConfigurationItem[]> = derived(
  businessConfigStore,
...
export const statusEstoqueStore: Readable<BusinessConfigurationItem[]> = derived(
  businessConfigStore,
 ...
 */
export function getConfigByCode(store: Readable<BusinessConfigurationItem[]>, code: string): Readable<BusinessConfigurationItem | null> {
  return derived(store, items => items.find(item => item.code === code) || null);
}

/**
 * Helper function para converter array de configuração em options para dropdowns
 */
export function configToOptions(items: BusinessConfigurationItem[], addEmptyOption = true): Array<{ value: string; label: string }> {
 ...
 */
export const tiposMovimentacaoOptions: Readable<Array<{ value: string; label: string }>> = derived(
  tiposMovimentacaoStore,
  items => configToOptions(items)
);

export const categoriasEPIOptions: Readable<Array<{ value: string; label: string }>> = derived(
  categoriasEPIStore,
  items => configToOptions(items)
);

export const statusEntregaOptions: Readable<Array<{ value: string; label: string }>> = derived(
  statusEntregaStore,
  items => configToOptions(items)
);

export const statusEstoqueOptions: Readable<Array<{ value: string; label: string }>> = derived(
  statusEstoqueStore,
  items => configToOptions(items)
);
