...
export interface ContratadaFilters {
  ...
  // Ordenação
  orderBy?: 'nome' | 'cnpj' | 'status' | 'dataInicioContrato' | 'totalColaboradores';
...
async function fetchContratadaData(params: any): Promise<PaginatedResponse<Contratada>> {
  try {
...
export const contratadaStats: Readable<{
  ...
  $data => {
    ...
    const totalColaboradores = $data.reduce((acc, c) => acc + (c.totalColaboradores || 0), 0);
    const mediaColaboradoresPorContratada = total > 0 ? totalColaboradores / total : 0;
    ...
    return {
      ...
      suspensas,
      totalColaboradores,
      mediaColaboradoresPorContratada: Math.round(mediaColaboradoresPorContratada * 100) / 100
    };
  }
);
...
export function sortContratadas(field: ContratadaFilters['orderBy'], direction: 'asc' | 'desc' = 'asc') {
  return contratadaStore.setSorting(field!, direction);
}

// ==================== CRUD OPERATIONS ====================

export async function createContratada(data: ContratadaCreateData): Promise<Contratada> {
  try {
...
export async function updateContratada(id: string, data: Partial<ContratadaCreateData>): Promise<Contratada> {
  try {
...
export async function validateCNPJWithBackend(cnpj: string): Promise<{
  ...
  try {
    ...
    return {
      ...
      exists: false,
      message: localValidation.message || (localValidation.valid ? 'CNPJ válido (verificação offline)' : 'CNPJ inválido')
    };
  }
}
