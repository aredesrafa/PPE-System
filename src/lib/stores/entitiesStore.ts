...
function createFichasStore() {
  ...
  return {
    ...
    update: (id: string, updates: Partial<FichaEPI>) => 
      update(fichas => fichas.map(ficha => 
        ficha.id === id ? { ...ficha, ...updates } : ficha
      )),
    remove: (id: string) => update(fichas => fichas.filter(ficha => ficha.id !== id)),
    ...
    getByColaborador: (colaboradorId: string): FichaEPI[] => {
      return get({ subscribe }).filter(ficha => ficha.colaboradorId === colaboradorId);
    },
    getAtivas: (): FichaEPI[] => {
...
function createColaboradoresStore() {
  ...
  return {
    subscribe,
    set,
    add: (colaborador: Colaborador) => update(colaboradores => [...colaboradores, colaborador]),
    update: (id: string, updates: Partial<Colaborador>) => 
      update(colaboradores => colaboradores.map(col => 
        col.id === id ? { ...col, ...updates } : col
      )),
    remove: (id: string) => update(colaboradores => colaboradores.filter(col => col.id !== id)),
...
function createEmpresasStore() {
  ...
  return {
    ...
    update: (id: string, updates: Partial<Empresa>) => 
      update(empresas => empresas.map(emp => 
        emp.id === id ? { ...emp, ...updates } : emp
      )),
    remove: (id: string) => update(empresas => empresas.filter(emp => emp.id !== id)),
...
function createEntregasStore() {
  ...
  return {
    ...
    update: (id: string, updates: Partial<Entrega>) => 
      update(entregas => entregas.map(ent => 
        ent.id === id ? { ...ent, ...updates } : ent
      )),
    remove: (id: string) => update(entregas => entregas.filter(ent => ent.id !== id)),
    ...
    getPendentes: (): Entrega[] => {
      return get({ subscribe }).filter(ent => ent.status === 'nao_assinado' || ent.status === 'pendente');
    },
    clear: () => set([])
  };
}
...
function createNotificacoesStore() {
  ...
  return {
    subscribe,
    set,
    add: (notificacao: Notificacao) => update(notifs => [notificacao, ...notifs]),
    update: (id: string, updates: Partial<Notificacao>) => 
      update(notifs => notifs.map(notif => 
        notif.id === id ? { ...notif, ...updates } : notif
      )),
    remove: (id: string) => update(notifs => notifs.filter(notif => notif.id !== id)),
...
export const estoqueDisponivel = derived(estoqueStore, $estoque => 
  $estoque.filter(item => item.quantidade > 0 && item.status === 'disponivel')
);
...
export const colaboradoresAtivos = derived(colaboradoresStore, $colaboradores => 
  $colaboradores.filter(col => col.status === 'ativo')
);

export const notificacoesNaoLidas = derived(notificacoesStore, $notificacoes => 
  $notificacoes.filter(notif => !notif.lida)
);

export const countNotificacoesNaoLidas = derived(notificacoesNaoLidas, $notificacoes => 
  $notificacoes.length
);
...
export const dashboardStats = derived(
  ...
  ([$estoque, $fichas, $colaboradores, $notificacoesNaoLidas]) => ({
    ...
    estoqueComProblemas: $estoque.filter(item => 
      item.status === 'baixo_estoque' || item.status === 'vencido' || item.status === 'esgotado'
    ).length
  })
);