...
export function createApiStore<T>(initialData: T | null = null) {
  ...
  return {
    ...
    set: (data: T) => store.update(state => ({ ...state, data, error: null })),
    setLoading: (loading: boolean) => store.update(state => ({ ...state, loading })),
    setError: (error: string | null) => store.update(state => ({ ...state, error, loading: false })),
...
export function createCachedApiStore<T>(cacheKey: string, initialData: T | null = null) {
