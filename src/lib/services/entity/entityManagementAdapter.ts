...
class EntityManagementAdapter {
   ...
   */
  async createContratada(data: Omit<ContratadaDTO, 'id' | 'createdAt' | 'updatedAt'>): Promise<ContratadaDTO> {
   ...
   * Atualiza contratada
   */
  async updateContratada(id: string, data: Partial<ContratadaDTO>): Promise<ContratadaDTO> {
   ...
   */
  async getColaboradores(params: EntityParams = {}): Promise<PaginatedColaboradores> {
   ...
   */
  async getColaboradorById(id: string, includeRelations = false): Promise<ColaboradorDTO> {
   ...
   */
  async createColaborador(data: Omit<ColaboradorDTO, 'id' | 'createdAt' | 'updatedAt'>): Promise<ColaboradorDTO> {
   ...
   */
  async updateColaborador(id: string, data: Partial<ColaboradorDTO>): Promise<ColaboradorDTO> {
   ...
   */
  async getTiposEPIByCategoria(categoria?: string, params: EntityParams = {}): Promise<TipoEPIDTO[]> {
   ...
   * Cria novo tipo EPI
   */
  async createTipoEPI(data: Omit<TipoEPIDTO, 'id' | 'createdAt' | 'updatedAt'>): Promise<TipoEPIDTO> {
   ...
   * Atualiza tipo EPI
   */
  async updateTipoEPI(id: string, data: Partial<TipoEPIDTO>): Promise<TipoEPIDTO> {
   ...
   */
  async getAlmoxarifados(params: EntityParams = {}): Promise<AlmoxarifadoDTO[]> {
    try {
      ...
      if (params.ativo !== undefined) {
        filteredData = mockAlmoxarifados.filter(item => item.ativo === params.ativo);
      }
   ...
   */
  async createAlmoxarifado(data: Omit<AlmoxarifadoDTO, 'id' | 'createdAt' | 'updatedAt'>): Promise<AlmoxarifadoDTO> {
   ...
   */
  async updateAlmoxarifado(id: string, data: Partial<AlmoxarifadoDTO>): Promise<AlmoxarifadoDTO> {
   ...
   */
  async getContratadaWithColaboradores(contratadaId: string): Promise<ContratadaDTO & { colaboradores: ColaboradorDTO[] }> {
   ...
   */
  async getContratadasWithStats(): Promise<Array<ContratadaDTO & { totalColaboradores: number; colaboradoresAtivos: number }>> {
    ...
    const contratadasWithStats = await Promise.all(
      contratadas.map(async (contratada) => {
        const colaboradores = await this.getColaboradoresByContratada(contratada.id);
   ...
   */
  async getTiposEPIMaisUtilizados(limit = 10): Promise<Array<TipoEPIDTO & { utilizacoes: number }>> {
