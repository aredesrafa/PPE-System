...
class NotesAdapter {
   ...
   */
  async getNotas(params: NotesFilterParams = {}): Promise<PaginatedResponse<Nota>> {
   ...
   * Cria uma nova nota
   */
  async createNota(data: CreateNotaData, tipo: 'entrada' | 'saida'): Promise<Nota> {
  ...
  async updateNota(id: string, data: UpdateNotaData): Promise<Nota> {
    ...
    try {
      ...
      const updatedNota: Nota = {
        ...existing,
        ...data,
        itens: data.itens ? data.itens.map((item, index) => ({
          ...item,
  ...
  async getFilterOptions(): Promise<{
    ...
    const responsaveis = [...new Set(data.items.map(item => item.responsavel))]
      .filter(Boolean)
      .sort()
  ...
  private getMockNotas(params: NotesFilterParams = {}): PaginatedResponse<Nota> {
    const mockNotasEntrada: NotaEntrada[] = [
      {
        ...
        valorTotal: 2500.00,
        itens: [
          { id: '1', tipoEPIId: '1', quantidade: 10, custoUnitario: 50.00 },
          { id: '2', tipoEPIId: '2', quantidade: 20, custoUnitario: 75.00 }
        ]
      },
      {
        id: '2',
        ...
        valorTotal: 1800.00,
        itens: [
          { id: '3', tipoEPIId: '3', quantidade: 15, custoUnitario: 120.00 }
        ]
      },
      {
        id: '3',
        ...
        valorTotal: 3200.00,
        itens: [
          { id: '4', tipoEPIId: '1', quantidade: 25, custoUnitario: 48.00 }
        ]
      }
    ];
    ...
    const mockNotasSaida: NotaSaida[] = [
      ...
      },
      {
        ...
        itens: [
          { id: '7', tipoEPIId: '3', quantidade: 10 }
        ]
      }
    ];
    ...
    if (params.responsavel) {
      allNotas = allNotas.filter(nota => nota.responsavel === params.responsavel);
    }

    if (params.search) {
      const searchLower = params.search.toLowerCase();
      allNotas = allNotas.filter(nota =>
        ...
        (nota.tipo === 'entrada' && nota.fornecedor?.toLowerCase().includes(searchLower)) ||
        (nota.tipo === 'saida' && nota.destinatario?.toLowerCase().includes(searchLower))
      );
    }
    ...
    allNotas.sort((a, b) => new Date(b.data).getTime() - new Date(a.data).getTime());
