...
class ProcessLifecycleAdapter {
   ...
   */
  async getFichasEPI(params: EntityParams = {}): Promise<PaginatedResponse<FichaEPIDTO>> {
   ...
   */
  async getFichaEPIById(id: string, includeRelations = true): Promise<FichaEPIDTO> {
   ...
   * Atualiza ficha EPI
   */
  async updateFichaEPI(id: string, data: Partial<FichaEPIDTO>): Promise<FichaEPIDTO> {
   ...
   */
  async getEntregas(params: EntityParams & {
    fichaEPIId?: string;
    status?: string;
    dataInicio?: string;
    dataFim?: string;
   ...
   */
  async getEntregaById(id: string, includeRelations = true): Promise<EntregaDTO> {
   ...
   * Atualiza entrega
   */
  async updateEntrega(id: string, data: Partial<EntregaDTO>): Promise<EntregaDTO> {
  ...
  async registrarAssinatura(entregaId: string, assinaturaData: {
    ...
    const result = await api.post<EntregaDTO>(`/entregas/${entregaId}/assinatura`, {
      ...assinaturaData,
   ...
   */
  async gerarLinkAssinatura(entregaId: string, expirationHours = 24): Promise<{
   ...
   */
  async validarAssinatura(entregaId: string, assinatura: string): Promise<{
    valid: boolean;
    reason?: string;
  }> {
   ...
   */
  async cancelarEntrega(entregaId: string, motivo: string): Promise<EntregaDTO> {
  ...
  async processarDevolucao(entregaId: string, devolucaoData: {
    ...
    const result = await api.post<EntregaDTO>(`/entregas/${entregaId}/devolucao`, {
      ...devolucaoData,
      dataDevolucao: new Date().toISOString()
    });
    
    // Log da transição de estado
    console.log(`🔄 Devolução processada para entrega ${entregaId}: ${devolucaoData.motivo}`);
   ...
   */
  async processarDevolucaoParcial(entregaId: string, data: {
    itens: Array<{
      itemId: string;
      quantidade: number;
      motivo: string;
   ...
   */
  async marcarItemPerdido(entregaId: string, itemId: string, data: {
    motivo: string;
    observacoes?: string;
  }): Promise<EntregaDTO> {
    return api.patch<EntregaDTO>(`/entregas/${entregaId}/itens/${itemId}/perdido`, {
      ...data,
   ...
   */
  async executeTransition(entregaId: string, transition: string, data?: any): Promise<EntregaDTO> {
    return api.post<EntregaDTO>(`/entregas/${entregaId}/transition/${transition}`, data);
  }
  
  /**
   * Busca histórico completo de transições
   */
  async getWorkflowHistory(entregaId: string): Promise<Array<{
    from: string;
    to: string;
    timestamp: string;
    userId: string;
   ...
   */
  async getProcessMetrics(params: {
    dataInicio?: string;
    dataFim?: string;
    colaboradorId?: string;
    fichaEPIId?: string;
   ...
   */
  async getDevolucoesPendentes(): Promise<Array<EntregaDTO & { diasSemDevolucao: number }>> {
   ...
   */
  async agendarNotificacaoDevolucao(entregaId: string, diasAntecedencia: number = 7): Promise<{
    success: boolean;
